...
export async function GET() {
  return withDatabase(async (db) => {
    try {
      ...
      const groupStockLevels = await db.$queryRaw<GroupStock[]>`
      ...
      const totalStock = groupStockLevels.reduce(
        (sum: number, group: GroupStock) => sum + Number(group.total_stock),
        0
      );

      console.log("\n\nTotal Stock:", totalStock.toString());

      const groupsWithPercentages = groupStockLevels.map(
        (group: GroupStock) => ({
